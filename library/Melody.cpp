 ...
 * Constructor
 *
 * param[in] pin The pin which is connected to the speaker
 */
...
/**
 * Function to mute the speaker
 */
void Melody::no_sound()
{
  pinMode(_pin, OUTPUT);
...
/**
 * Function to produce sound at a certain frequency
 * param[in] frequency The frequency we want to play
 *           duration The duration which we want the note to be played
 */  
void Melody::sound(uint16_t frequency, uint32_t duration) {
  int period = 1000000L / frequency;
  int pulse = period / 2;
  pinMode(_pin, OUTPUT);
  for (long i = 0; i < duration * 5000L; i += period) 
  {
 ...
 *
 * param[in] note The array consisting of the notes to be played
 *           note_duration The array consisting of the duration of the notes
 *           bpm The speed in which we want the music to be played at
 */           
void Melody::play_music(uint16_t note[], uint16_t note_duration[], uint8_t bpm) {
  uint16_t len = 0;
  uint16_t i = 0;
  float speed_factor = 28.0 / bpm;
  while (note[len] != 1) {
    len++;
  }
  for (i = 0; i < len; i++) {
    sound(note[i], note_duration[i] * speed_factor);
...
/**
 * Various melody tunes that can be played
 */ 
void Melody::little_happiness() {
  uint16_t note_A[] = {NOTE_A4, NOTE_A4, NOTE_C5, NOTE_C5, NOTE_F5, NOTE_F5, NOTE_E5, NOTE_E5,
  ...
  uint16_t note_duration_A[] = {QUA, QUA, QUA, QUA, QUA, QUA, QUA, QUA, QUA, QUA, MNM, QUA,
                                QUA, QUA, QUA, QUA, QUA, QUA, QUA, QUA, QUA, QUA, MNM, QUA,
  ...
  uint16_t note_B[] = {NOTE_C5, NOTE_A5, NOTE_G5, NOTE_F5, NOTE_G5, NOTE_A5, NOTE_C5, NOTE_G5, 
  ...
  uint16_t note_duration_B[] = {QUA, QUA, SEMIQ, QUA + SEMIQ, QUA, QUA, QUA, QUA, CRO, QUA, QUA,
                                ...
                                QUA, QUA, QUA, QUA + SEMIQ, QUA + SEMIQ, CRO};
...
void Melody::laputa() {
  uint16_t note[] = {NOTE_A4, NOTE_B4, NOTE_C5, NOTE_B4, NOTE_C5, NOTE_E5, NOTE_B4, REST,
  ...
  uint16_t note_duration[] = {QUA, QUA, CRO + QUA, QUA, CRO, CRO, MNM, CRO, CRO, CRO + QUA,
...
void Melody::kahoot() {
  uint16_t note[] = {NOTE_C4, NOTE_C4, NOTE_F4, NOTE_F4, NOTE_GS4, NOTE_GS4, NOTE_F4, NOTE_F4,
  ...
  uint16_t note_duration[] = {QUA, QUA, QUA, QUA, QUA, QUA, QUA, QUA, CRO, QUA, QUA,
...
void Melody::konosuba_end() {
  uint16_t note_A[] = {NOTE_C5, NOTE_C5, NOTE_D5, NOTE_E5, NOTE_E5, NOTE_G5, NOTE_A5, NOTE_C6, 
                       NOTE_A5, NOTE_G5, NOTE_E5, NOTE_E5, NOTE_D5, NOTE_C5, NOTE_A5, NOTE_G5,
                       NOTE_E5, NOTE_D5, NOTE_G4, NOTE_C5, NOTE_C5, NOTE_D5, NOTE_C5, 1};
                
  uint16_t note_B[] = {NOTE_C5, NOTE_C5, NOTE_D5, NOTE_E5, NOTE_E5, NOTE_G5, NOTE_A5, NOTE_C6, 
                       NOTE_A5, NOTE_G5, NOTE_E5, NOTE_E5, NOTE_D5, NOTE_C5, NOTE_C5, NOTE_D5, 
                       NOTE_E5, NOTE_E5, NOTE_G5, NOTE_A5, NOTE_G5, NOTE_E5, NOTE_D5, 1};

  uint16_t note_C[] = {NOTE_E5, NOTE_F5, NOTE_G5, NOTE_A5, NOTE_G5, NOTE_A5, NOTE_G5, NOTE_E5,
  ...
  uint16_t note_duration_A[] = {CRO + QUA, CRO, QUA, CRO + QUA, CRO, QUA, CRO, QUA, CRO, QUA, CRO + QUA, 
                                CRO, QUA, CRO + QUA, CRO + QUA, CRO, QUA, CRO, QUA, CRO + QUA, 
                                CRO, QUA, MNM + CRO};

  uint16_t note_duration_B[] = {CRO + QUA, CRO, QUA, CRO + QUA, CRO, QUA, CRO, QUA, CRO, QUA, CRO + QUA, 
                                CRO, QUA, CRO + QUA, CRO, QUA, CRO + QUA, CRO, QUA, CRO + QUA, 
                                CRO, QUA, MNM + CRO};

  uint16_t note_duration_C[] = {CRO, QUA, MNM + QUA, QUA, MNM + QUA, QUA, CRO, QUA, CRO, QUA,
...
void Melody::moonlight() {
  uint16_t note[] = {NOTE_G5, NOTE_G5, NOTE_G5, NOTE_G5, NOTE_E5, NOTE_C5, NOTE_D5,
                     ...
                     NOTE_A5, NOTE_G5, NOTE_C5, NOTE_D5, NOTE_E5,
                     NOTE_C5, NOTE_C6, NOTE_D6, NOTE_C6, NOTE_A5,
                     NOTE_E5, NOTE_G5, NOTE_A5, NOTE_G5, NOTE_A5,
                     NOTE_C6, NOTE_A5, NOTE_G5, NOTE_E5, NOTE_D5,
                     NOTE_E5, NOTE_G5, NOTE_E5, NOTE_D5, NOTE_C5, NOTE_E5, NOTE_D5,
                     1};                                       //NOTE_A4, NOTE_C5

  uint16_t note_duration[] = {QUA, QUA, QUA, CRO, QUA, QUA, QUA, CRO, MNM, QUA,
                              QUA, QUA, QUA, QUA, CRO, SEMIQ, SEMIQ, MNM, QUA,
                              QUA, QUA, QUA, CRO, QUA, QUA, QUA, CRO, CRO, QUA,
                              QUA, QUA, QUA, QUA, CRO, QUA, MNM, QUA, QUA, QUA, 
							  QUA, CRO, QUA, QUA, QUA, CRO, QUA + CRO, QUA,
                              QUA, QUA, QUA, QUA, QUA, CRO, CRO, MNM, QUA, QUA,
                              QUA, QUA, CRO, QUA, QUA, QUA, CRO, CRO + QUA, QUA, QUA,
						      QUA, QUA, QUA, QUA, CRO, QUA, MNM,/**/CRO, QUA,
                              QUA, CRO, CRO, QUA + SEMIQ, QUA + SEMIQ, QUA + SEMIQ, 
                              CRO + QUA, CRO, QUA, QUA, CRO, CRO, MNM, QUA + SEMIQ, 
							  QUA, QUA, CRO, QUA, MNM, MNM, QUA, QUA, QUA, QUA, QUA, 
							  CRO, QUA, SEMIBREVE};
...
void Melody::mary_had_a_little_lamb() {
  uint16_t note[] = {NOTE_E5, NOTE_D5, NOTE_C5, NOTE_D5, NOTE_E5, NOTE_E5, 
                     NOTE_E5, NOTE_D5, NOTE_D5, NOTE_D5, NOTE_E5, NOTE_G5, 
                     NOTE_G5, NOTE_E5, NOTE_D5, NOTE_C5, NOTE_D5, NOTE_E5, 
                     NOTE_E5, NOTE_E5, NOTE_D5, NOTE_D5, NOTE_E5, NOTE_D5,
                     NOTE_C5, 1};
                
  uint16_t note_duration[] = {CRO, CRO, CRO, CRO, CRO, CRO, MNM, CRO, 
                              CRO, MNM, CRO, CRO, MNM, CRO, CRO, CRO, 
                              CRO, CRO, CRO, MNM, CRO, CRO, CRO, CRO, MNM};
...
void Melody::merry_christmas() {
  uint16_t note[] = {NOTE_C5, NOTE_F5, NOTE_F5, NOTE_G5, NOTE_F5, NOTE_E5, NOTE_D5,
  ...
  uint16_t note_duration[] = {CRO, CRO, QUA, QUA, QUA, QUA, CRO, CRO,
...
void Melody::happy_birthday() {
  uint16_t note[] = {NOTE_G5, NOTE_G5, NOTE_A5, NOTE_G5, NOTE_C6, NOTE_B5, NOTE_G5, 
  ...
  uint16_t note_duration[] = {QUA + SEMIQ, SEMIQ, CRO, CRO, CRO, MNM,
...
void Melody::fur_elise() {
  uint16_t note[] = {NOTE_E5, NOTE_DS5, NOTE_E5, NOTE_DS5, NOTE_E5, NOTE_B4, NOTE_D5, 
...
void Melody::korobeiniki() {
  uint16_t note[] = {NOTE_E5, NOTE_B4, NOTE_C5, NOTE_D5, NOTE_E5, NOTE_D5, NOTE_C5,
  ...
  uint16_t note_duration[] = {CRO, QUA, QUA, QUA, SEMIQ, SEMIQ, QUA, QUA, CRO, QUA,
...
void Melody::random_tune() {
  ...
  switch(random(1, 12)) {
    ...
    case 7: moonlight(); break;
    case 8: konosuba_end(); break;
    case 9: kahoot(); break;
    case 10: laputa(); break;
    case 11: little_happiness(); break;
  }
}
